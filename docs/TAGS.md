...
| TRONC   | Tècnica                    | `tecnica`         | `direccio-tronc`, `direccio-xicalla` | afegida per `1784900000000-AddTecnicaTag`; `direccio-figura` → `direccio-tronc` per `1785100000000-UnifyAndRenameDirectionZones` |
...
`comodin`), direccions (`direccio-tronc`, `direccio-xicalla` — zona `DIRECTION`), decoració i
el valor solt `base` (`BASE_POSITION_TYPE`).
...
direcció; després de `1785100000000-UnifyAndRenameDirectionZones` són `direccio-tronc` i
`direccio-xicalla`); el seu `down` només la lleva si no té ningú assignat.
