...
| 3. Architecture                                                                                         | —                                                                                          | 3 (3 ✅)       | 8 (6 ✅)             | —                 | 11 (9 ✅)            |
| 4. Code smells                                                                                          | —                                                                                          | 1 (1 ✅)       | 11 (10 ✅, 1 🚫)     | 3 (3 ✅)          | 15 (14 ✅, 1 🚫)     |
...
| **Total (this document)**                                                                               | **5 (5 ✅)**                                                                                | **26 (26 ✅)** | **35 (30 ✅, 3 🚫)** | **7 (6 ✅, 1 🚫)** | **73 (67 ✅, 4 🚫)** |
...
### 🟡✅ ARCH-4 — `timestamp` (without time zone) everywhere — FIXED
...
**Fix applied:** every explicit `type: 'timestamp'` column and every `@CreateDateColumn()`/`@UpdateDateColumn()` decorator (which defaults to `timestamp` too) across all 15 entities with date columns now declares `type: 'timestamptz'`. Added migration `1783000000000-ConvertTimestampsToTimestamptz` which `ALTER`s all 34 affected columns (across `refresh_tokens`, `users`, `persons`, `events`, `attendances`, `figure_templates`, `figure_nodes`, `rengles`, `compositions`, `event_segments`, `figure_instances`, `instance_nodes`, `node_assignments`, `seasons`, `positions`) to `timestamptz` `USING col AT TIME ZONE 'UTC'` — lossless since app and DB have always shared UTC. Verified the migration runs cleanly against a real Postgres instance via `nx run api:test-integration`, and the full unit suite (746 tests) still passes unchanged.

### 🟡✅ ARCH-5 — Inconsistent 401 vs 403 in the refresh flow — FIXED

`POST /auth/refresh` threw `ForbiddenException` (403) when the cookie is missing but `UnauthorizedException` (401) for invalid/expired/reused tokens. Clients had to special-case both.

**Fix applied:** the missing-cookie branch in `AuthController.refresh` now throws `UnauthorizedException` too, so the endpoint is consistently 401 for every "not authenticated" case. Covered by the existing `auth.controller.spec.ts` test (updated to assert `UnauthorizedException`).

### 🟡✅ ARCH-6 — Duplicate soft-delete paths on Person — FIXED

`softDelete` (204, used by `DELETE /persons/:id`) and `deactivate` (200 + DTO, used by `PATCH /persons/:id/deactivate`) did the same thing with a different response shape/status — two code paths to maintain for one concept.

**Fix applied:** neither endpoint was actually called from the dashboard frontend at the time of the fix, so the tie-break rule (keep `DELETE`) was applied — `PATCH /persons/:id/deactivate` and `PersonService.deactivate` were removed; `DELETE /persons/:id` (`softDelete`) is now the only soft-delete path. The `lastSyncedAt`-preservation regression test that covered `deactivate` was ported to `softDelete` in `person.service.spec.ts` so that coverage isn't lost.

### 🟡✅ ARCH-7 — No graceful shutdown — FIXED

`main.ts` never called `app.enableShutdownHooks()`. Under Docker, SIGTERM would kill the process without letting NestJS close the TypeORM pool / in-flight requests — relevant with `restart: unless-stopped` and healthchecks in prod compose.

**Fix applied:** `app.enableShutdownHooks()` is now called right after `NestFactory.create(...)` in `main.ts`.

### 🟠✅ ARCH-8 — Query patterns that won't scale: cartesian joins and N+1 loops — PARTIALLY FIXED (correctness parts; performance parts deliberately left open)

- 🔵 **Deliberately left open — pure performance, not worth it at this scale.** `node-assignment.service.ts:574-593` (`getHistory`): one query `leftJoinAndSelect`s `assignments` × `instanceNodes` per instance — a **cartesian product** (an instance with 80 assignments and 100 nodes yields 8 000 joined rows, × 20 instances per page). Same shape in `getEventAssignmentSummary`, which additionally runs it **per segment** in a loop. A real colla tops out around ~1000 members, so a single figure's node/assignment counts are nowhere near the illustrative worst case; verified via `node-assignment-raw-queries.integration.spec.ts` (added for TEST-2) that TypeORM's join-to-entity re-hydration is correct (no duplicated/lost counts from the fanout) — the only remaining concern is bytes-over-wire, which doesn't matter here. Revisit only if colla size assumptions change.
- ✅ **Non-atomicity addressed — full transactional rewrite explicitly scoped out.** `bulkImport` (`node-assignment.service.ts:901+`): per source assignment it runs 3 conflict queries and then calls `assign()`, which itself re-runs the lock check, instance fetch, person fetch and the same 3 conflict checks plus a reload — ~10 queries × N assignments, i.e. ~1 000 queries to import a full pinya. It is also **not transactional**, so a crash mid-import leaves a half-imported figure. **Investigated and deliberately not wrapped in one transaction:** doing so properly would require threading an optional `EntityManager` through `assign()`/`snapshotInstance()` (both operate on injected repositories bound to the default connection, so calling them inside `bulkImport`'s own transaction wouldn't make their queries participate in it) — a much larger, riskier change to the module's most correctness-critical method than the original finding implied, and it would also change `bulkImport`'s intentional partial-success semantics (one unrelated failure would roll back every already-succeeded entry instead of just reporting it as a conflict). Investigation found `bulkImport` is **already naturally resumable**: its own conflict checks (node/person/segment occupied) mean a request that dies mid-loop can simply be retried — already-imported entries come back as harmless conflicts, and the rest complete. This resumability is now covered by a new regression test (`node-assignment-raw-queries.integration.spec.ts`, "is safe to retry after a simulated crash mid-import") proving a retry after a simulated mid-import crash reports the already-done entry as a conflict, completes the rest, and leaves no duplicates.
- 🔵 **Deliberately left open — pure performance, not worth it at this scale.** `ProjectionService.getProjection` awaits `getInstanceNodes` + `getByInstance` sequentially per instance. A segment has a handful of figures at most; sequential vs. parallel awaits here is negligible. Revisit only if segments grow far beyond current usage.
- ✅ **FIXED (the race; the code-smell filtering was left as-is).** `AttendanceService.recalculateSummary` loads all attendances (with persons) into memory and filters the array 7 times, instead of one `GROUP BY` query; concurrent attendance writes raced on the denormalized `event.attendanceSummary` (last write wins with stale data) — this part wasn't a scale problem, it was a genuine correctness bug independent of colla size. **Fix applied:** `recalculateSummary` now runs inside `dataSource.transaction(...)` and takes a `pessimistic_write` lock on the event row (`manager.createQueryBuilder(Event, 'event').setLock('pessimistic_write')...getOne()`) *before* reading attendances, so two concurrent recalculations for the same event serialize instead of interleaving — the second one's read is guaranteed to see everything the first one committed. The in-memory filter-7-times computation itself was left unchanged (still a minor smell, just no longer racy) — rewriting it as a single aggregate query was considered (see the earlier discussion) and rejected as unnecessary churn at this scale, given the race was the only real correctness concern. Covered by new unit specs in `attendance.service.spec.ts` (transaction is used; lock is acquired via the correct query-builder call, before the attendance read) and a new `attendance-summary-race.integration.spec.ts` proving, against a real Postgres instance, that 10 concurrent `create()` calls for the same event never lose an update (`attendanceSummary.total` always ends at exactly 10) — the exact class of bug a mocked-repository test structurally cannot demonstrate.

None of the two performance-only bullets are wrong at ~1000 persons/colla scale — they're the endpoints that would degrade first at a much larger scale, deliberately left alone. The two correctness-flavored bullets (`bulkImport`'s crash-safety, the attendance summary race) have been investigated/addressed above.
...
- 🟡✅ **SM-1** — **FIXED.** `user.entity.ts:13-14`: `type PersonRef = any` to dodge a circular import. **Fix applied:** replaced with `import type { Person }` + TypeORM's `Relation<Person>`, which breaks the same circular-import cycle without erasing the type.
...
- 🟡✅ **SM-3** — **FIXED.** `auth.service.ts:176-179`: raw SQL `UPDATE users SET person_id = ...` inside an otherwise repository-based service; bypassed entity hooks and `updatedAt`, and had no existence check on `personId` (FK violation → 500). **Fix applied:** `createWithInvite`/setup flow now checks `personRepo.findOne` up front and throws `NotFoundException` if the `personId` doesn't exist, then links the relation via `manager.update(User, saved.id, { person: { id: personId } })` instead of raw SQL.
- 🟡✅ **SM-4** — **FIXED.** `main.ts:1-4`: scaffold comment “This is not a production server yet!” on a production API; `const cookieParser = require('cookie-parser')` instead of an ES import. **Fix applied:** removed the stale comment and switched to `import cookieParser from 'cookie-parser'`.
...
- 🔵✅ **SM-7** — **FIXED.** `AuthController.login` typed `req.user` inline and then cast with `Parameters<typeof this.authService.login>[0]` — noisy. **Fix applied:** replaced with a small `RequestWithUser extends Request { user: User }` interface.
...
- 🟡✅ **SM-9** — **FIXED.** Pervasive `(x as any)` casts to reach `Person` fields (`node-assignment.service.ts:182-187,602,611,720,813` …) — a consequence of `type`-only imports erasing entity types. **Fix applied:** fixing the `Person`/`User` entity relation typing (SM-1) removed the need for every cast; all `(x as any)` accesses in `node-assignment.service.ts` are now plain typed property access.
- 🟡✅ **SM-10** — **FIXED.** `figure-template.service.ts` `handleDbError`: any non-unique-violation DB error became a bare 500 "Unexpected database error" **without logging the original error** — undiagnosable in production. **Fix applied:** added a `Logger` instance to `FigureTemplateService` and log the original error (`this.logger.error(err)`) before throwing the generic `InternalServerErrorException`. Covered by a new spec asserting `Logger.prototype.error` is called with the raw DB error.
- 🟡✅ **SM-11** — **FIXED.** Template editor saves (`update` → `syncNodes` + `syncRengles`) and `saveFromInstance` ran multiple dependent writes **without a transaction** — a failure mid-save left the template half-updated (nodes updated, deletions skipped, rengles inconsistent). **Fix applied:** `FigureTemplateService` now injects `DataSource`; `update()` wraps the template save + `syncNodes` + `syncRengles` in one `dataSource.transaction`, and `saveFromInstance()` wraps both the `overwrite` path (`syncNodes`) and the `new_version` path (new template + copied rengles + copied nodes) each in their own transaction. `createNodes`/`syncNodes`/`syncRengles` now take an optional scoped `Repository` (defaulting to the instance repositories) so they operate on the transaction's `EntityManager` when called from within one. Covered by new specs asserting a single `dataSource.transaction` call wraps all three writes, and that a template-save failure prevents any node/rengla writes from running.
- 🟡✅ **SM-12** — **FIXED.** `available-persons.service.ts:80-89`: manual re-coercion of `isXicalla`/`excludeAssigned` from strings, duplicating the `@Transform` logic in `AvailablePersonsQueryDto` — except the DTO was never actually wired in: the controller declared `@Query() query: AvailablePersonsQuery` (a plain TS interface), which NestJS's global `ValidationPipe` can't reflect a metatype for, so it skipped transformation entirely and raw string query params reached the service, which is why the manual coercion existed in the first place. **Fix applied:** the controller now types the query param as `AvailablePersonsQueryDto` (added the missing `positionId` field to the DTO so `forbidNonWhitelisted` doesn't reject it), so the global pipe's `transform: true` now actually coerces `isXicalla`/`excludeAssigned` to real booleans (the DTO's `@Transform` was tightened to preserve `undefined` when the param is absent, rather than coercing it to `false`, so the service's existing `?? true` default for `excludeAssigned` still applies). The dead manual coercion block in the service was deleted. Covered by a new `available-persons-query.dto.spec.ts` asserting the transform behavior, plus the existing service specs (unaffected, since they already called the service with real booleans).
- 🟡🚫 **SM-13** `EventService.remove` checks attendance count with a separate query instead of relying on FK behavior, while figure data cascades (see ARCH-10) — the protection level is accidental, not designed. **Won't fix** (skipped at user's request).
- 🔵✅ **SM-14** — **FIXED.** `FigureTemplateService.create` silently renamed duplicates (`"Nom 2"`) while `update` threw 409 for the same situation. **Fix applied:** `create` now calls the same `assertNameAvailable` helper `update` uses and throws `ConflictException` on a duplicate name instead of auto-suffixing; the now-dead `generateUniqueName` helper was removed. The dashboard's template editor already treats any 409 whose message mentions "name" as a duplicate-name conflict toast, so no frontend change was needed. Covered by an updated spec asserting the 409 and that `save` is never reached.
- 🔵✅ **SM-15** — **FIXED.** `AttendanceService.update` overwrote `respondedAt` on every edit, including notes-only edits — the "responded" timestamp lost its meaning. **Fix applied:** `respondedAt` is now only bumped when `dto.status` is present and differs from the current status; a notes-only edit leaves it untouched. Covered by new specs asserting `respondedAt` is unchanged on a notes-only update and is bumped when the status actually changes.
