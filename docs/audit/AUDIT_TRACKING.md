...
| WI-03 | **Normalitzar tap targets ≥24px** (chips, checkboxes, enllaços, badges) | Transversal | 🟠 | M | PE-L1, PE-L2, EV-M3, CF-L1, PW-L3, PW-L4 | ✅ | 🟣 | `fix/audit-tap-targets` (pendent d'obrir PR — agrupada amb WI-13 + neteja del tauler, 3 commits) |
...
| WI-05 | **Formulari Persona**: apilar etiqueta/input en mòbil | Persons | 🟠 | S | PE-M3(layout) | ✅ | ✔️ | [#90](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/90) (fusionada a develop) |
| WI-06 | **Detall Persona**: `flex-wrap` a les barres de botons (no tallar en mòbil) | Persons | 🟠 | S | PE-M2 | ✅ | ✔️ | [#89](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/89) (fusionada a develop) |
...
| WI-08 | **Detall Event**: assistència com a llista de fitxes en mòbil | Events | 🟠 | M | EV-M2 | ✅ | ✔️ | [#93](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/93) (fusionada a develop) |
| WI-09 | **Nav tablet**: etiquetes visibles / tooltip real (no només icones) | Shell | 🟠 | S | UX-M4 | ✅ | ✔️ | [#91](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/91) (fusionada a develop) |
| WI-10 | **Pinyes**: arreglar pestanya **Distribució** en mòbil (`aside` fluid + canvas mai 0px) | Pinyes | 🔴 | M | P-H1 | ✅ | ✔️ | [#88](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/88) (fusionada a develop) |
...
| WI-12 | **Pinyes**: barra de 5 pestanyes scrollable en mòbil | Pinyes | 🟠 | S | P-M1 | ✅ | ✔️ | [#92](https://github.com/MuixerangaBarcelonaCat/MuixerApp/pull/92) (fusionada a develop) |
| WI-13 | **Pinyes**: en mòbil, missatge "no disponible" al workspace d'assignació (abast reduït — abans: canvas full-width + drawer) | Pinyes | 🟠 | S | P-M2, GE-H3 | ✅ | 🟣 | `fix/audit-tap-targets` (pendent d'obrir PR — agrupada amb WI-03) |
...
| WI-21 | **Teclat numèric de confirmació d'assistència** massa petit en mòbil (`w-[calc((60vw-6.5rem)/10)]` dona ~13px/tecla a 393px; la fórmula només funciona bé en viewports amples) | Events | 🟠 | S | RE-01 (nova) | 🆕 | ⬜ | — |
| WI-22 | **Inputs de cerca/filtre transversals** (`label.input > input.grow.bg-transparent`) fan 20px d'alçada real (< 24px WCAG) a Persons, Events (assaigs+actuacions), Config/Users i Pinyes (template-list, composition-editor) | Transversal | 🟡 | S | RE-02 (nova) | 🆕 | ⬜ | — |
| WI-23 | **`/persons/new` fa una crida 400** a `GET /api/persons/new` (+ history): no hi ha ruta dedicada per crear, `:id` matcha literalment `"new"` com si fos un ID. Inofensiu per l'usuari (formulari buit igualment) però genera soroll de consola a cada alta | Persons | 🟡 | XS | RE-03 (nova) | 🆕 | ⬜ | — |
...
| 2026-07-22 | WI-03 | ✅ Aprovat → 🟣 En PR | Fix transversal d'accessibilitat (WCAG tap targets <24px), fet l'últim tal com es va decidir. **Fixes aplicats (verificats empíricament al navegador amb `getBoundingClientRect`, no només per classe CSS):** (1) `ActiveFiltersComponent` (compartit — arregla PE-L1+CF-L1 a totes les llistes d'un cop): botó "×" `min-h-6 min-w-6 inline-flex items-center justify-center` → 24×24px real. (2) `person-list` checkbox de filtre per etiqueta: `min-h-6` a la `<label>` embolcalladora (el checkbox visual es manté xicotet, l'àrea clicable creix). (3) `person-detail` (PE-L2): badges de posició `min-h-6 inline-flex items-center` (20→24px) i toggles Actiu/Membre/Xicalla `toggle-sm`→`toggle` (20→24px, mida per defecte de DaisyUI). (4) `event-detail` (EV-M3), taula i targetes (WI-08): badge d'estat d'assistència i badges de filtre per etiqueta `min-h-6 inline-flex items-center`; enllaços d'àlies/nom. (5) `season-list` (CF-L1): enllaç del recompte d'events. (6) PWA: `input` de login amb `h-6` explícit (el wrapper ja era prou gran, però el control real només ocupava la línia de text); enllaç d'ubicació al detall d'event. **Troballa important durant la verificació:** la primera implementació dels enllaços nus (`<a>`/`<span class="link">`) usava `leading-6` (line-height 24px) — semblava correcte per classe i per `getComputedStyle`, però l'alçada REAL (`getBoundingClientRect`) seguia sent 17px, perquè `line-height` no infla la caixa mesurable d'un element `inline`. Corregit a `inline-flex items-center min-h-6` a tots els enllaços afectats (event-detail ×4, season-list, PWA event-detail), reverificat: 24px real. **Verificat com a JA conforme, sense canvis:** `<th>` d'ordenació de `app-data-table` (padding de `.table-sm` + text ≈32px, molt per sobre de 24px) i els checkboxes `btn btn-sm` de `user-list`/`person-list` (label sencera ja fa ≥30px, el checkbox visual de 16px no és l'únic target clicable). **Efecte col·lateral:** en tocar `person-detail.component.spec.ts` es va detectar que el test de WI-05 (apilar etiquetes) havia desaparegut durant les fusions en paral·lel anteriors (el fix de codi seguia intacte, només el test es va perdre) — restaurat en aquest mateix commit. Verificat: suite dashboard 1316/1318 (2 skipped previs) i pwa 110/110, lint 0 errors als dos apps, build ✓. Branca `fix/audit-tap-targets`. |
| 2026-07-22 | WI-05 | ✅ Aprovat → ✔️ Fet | Formulari Persona (PE-M3, part de layout): els 10 camps `form-control > label.label` (Nom, cognoms, Àlies, Telèfon, Data de naixement, Alçada espatlles, Disponibilitat, Acollida, Data camisa) usen el `.label` de DaisyUI (fila per defecte, etiqueta-esquerra/input-dreta) i les etiquetes llargues trencaven en 2-3 línies estrenyent el camp. **Fix:** `flex-col items-start gap-1` per sota de `sm`, `sm:flex-row sm:items-center sm:gap-0` per mantenir el layout original a partir de `sm`. TDD: 1 test nou a `person-detail.component.spec.ts` (totes les labels amb `flex-col`/`sm:flex-row` quan `editing()=true`), vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 393px i 1280px (sense regressió). **Fusionada a develop via PR #90.** |
| 2026-07-22 | WI-06 | ✅ Aprovat → ✔️ Fet | Detall Persona (PE-M2): la fila de botons de capçalera ("Marcar provisional"/"Edita") i la de la secció "Informació de la colla" ("Envia correu d'invitació"/"Enllaça amb usuari existent") es tallaven per la dreta a 393px. **Fix:** `flex-wrap` a les 3 files implicades (capçalera exterior, grup de botons de capçalera, fila de botons de la colla); de pas, corregit `items-left` (classe Tailwind inexistent) → `items-start` a la mateixa fila. TDD: 3 tests nous a `person-detail.component.spec.ts`, vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 393px (botons apilats sense tallar) i 1280px (sense regressió, mateixa disposició d'abans). **Fusionada a develop via PR #89.** |
| 2026-07-22 | WI-08 | ✅ Aprovat → ✔️ Fet | Llista d'assistència com a targetes en mòbil al detall d'Event (EV-M2): la taula de 557px (5 columnes) scrollejava horitzontalment a 393px. **Disseny:** taula feta a mà (fora d'abast de WI-02); es reutilitza el patró `cardMode` (`matchMedia('(max-width:1023.98px)')`) amb un bloc de targetes dedicat, reutilitzant `getStatusLabel`/`getStatusBadgeClass`/`navigateToPerson`/`filterByPosition`/`openAttendanceEdit`. TDD: primer TestBed complet del component (abans només tests purs), 3 tests nous, vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 393px (targetes) i 1280px (taula intacta). **Fusionada a develop via PR #93.** |
| 2026-07-22 | WI-09 | ✅ Aprovat → ✔️ Fet | Etiquetes de navegació visibles en tablet (UX-M4): `TabNavComponent` amagava l'etiqueta fins a `lg` (1024px) i només mostrava un `sr-only` (mai visible en tàctil) entre `sm` i `lg`. **Fix:** breakpoint baixat de `lg` a `md` (768px) — `hidden md:inline` per l'etiqueta visible, `md:hidden sr-only` per l'accessible nom entre `sm` i `md` (640-767px, encara icona sola). Sense JS/matchMedia, purament CSS Tailwind, consistent amb el patró existent del component. TDD: nou `tab-nav.component.spec.ts` (no existia cap test previ), vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 700px (icona sola, esperat), 768px (6 etiquetes visibles sense overflow), 1280px (sense regressió). **Fusionada a develop via PR #91.** |
| 2026-07-21 | WI-10 | ✅ Aprovat → ✔️ Fet | Severitat Alta: `aside` d'amplada fixa no cap a 393px i empeny el canvas fora de pantalla → Konva llança `InvalidStateError` (canvas amb amplada/alçada 0). **Fix de layout:** contenidor `flex flex-col sm:flex-row` (abans sempre fila); `aside` `w-full max-h-[45vh]` en mòbil, `sm:w-[280px]` en tablet/desktop. **Fix defensiu:** `FigureCanvasComponent.resizeStage()` ignora resize a 0×0 (mai crida `stage.width(0)`/`height(0)`), com a xarxa de seguretat independent del layout. **Troballa col·lateral:** la classe original `w-70` no existeix a l'escala d'espaiat per defecte de Tailwind (salta de 64 a 72) i mai havia generat CSS — confirmat buidant `dist/apps/dashboard/browser/styles-*.css` després d'un build net; per això calia `w-[280px]` (sintaxi arbitrària) en lloc de `w-70`. El mateix patró trencat existeix a `composition-editor.component.html` (2 asides) — fora d'abast d'aquest WI, delegat com a tasca separada. TDD: 2 tests nous a `distribucio-tab.component.spec.ts` (classes responsive del contenidor i de l'aside), vermell→verd. Verificat: suite dashboard 1298/1298 ✓, lint 0 errors, build ✓, i confirmació visual al navegador (393px apilat sense error de consola; 1024px fila costat a costat amb aside de 280px, sense regressió). **Fusionada a develop via PR #88.** |
| 2026-07-22 | WI-12 | ✅ Aprovat → ✔️ Fet | Barra de 5 pestanyes desborda en mòbil (P-M1): `nav.tabs` (455px de contingut) no tenia scroll ni col·lapse, i "Distribució" quedava tallada a mitges. **Fix:** `flex-nowrap overflow-x-auto min-w-0` al `nav`, `shrink-0` al grup de navegació prev/next (perquè sigui el `nav` qui s'encongeixi i faci scroll, no els controls de segment). Sense breakpoint: a desktop/tablet ja cap sencer, `overflow-x-auto` és inert. TDD: 2 tests nous a `segment-workspace.component.spec.ts`, vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual+JS a 393px (scrollWidth 454 > clientWidth 52, `scrollLeft` revela "Distribució") i 1280px (les 5 pestanyes senceres, sense regressió). **Fusionada a develop via PR #92.** |
...
| 2026-07-22 | WI-13 | ✅ Aprovat → 🟣 En PR | **Re-mesura feta abans d'implementar** (al navegador, sessió autenticada, `getBoundingClientRect` a les pestanyes Pinyes/Troncs): a 393px el canvas fa **73px** exactes — GE-H3 correcta, P-M2 (~200px) estava obsoleta. A 640px (`sm`) el canvas ja fa 320px (usable); a 768px, 448px. **Llindar triat: `sm` (639.98px)**, consistent amb la resta de breakpoints d'esta auditoria. **Implementació:** `mobileUnsupported` signal (patró `matchMedia` idèntic a WI-08/WI-10) a `PinyesTabComponent` i `TroncsTabComponent` (totes dues pateixen el mateix problema: panell de persones `w-80` fix + canvas `flex-1` que es queda sense espai) — per sota de `sm`, substitueix tot el contingut (canvas+panell+footer) per un missatge "Encara no optimitzat per a mòbil". Tablet/escriptori sense canvis. TDD: 2 tests nous a cada component (`pinyes-tab` i `troncs-tab`), vermell→verd. Verificat: suite dashboard verda (1320/1322, 2 skips previs — l'avís puntual de "flaky task" de Nx Cloud resulta ser un historial agregat, no una fallada real d'esta execució, confirmat repetint 4 vegades amb 0 errors), lint 0 errors, build ✓, confirmació visual a 393px (missatge a Pinyes i Troncs) i 1280px (sense regressió a cap de les dues pestanyes). Branca `fix/audit-pinyes-workspace-mobile-guard`. |
...
| 2026-07-22 | Re-auditoria | — | **Repàs amb Playwright de tot el treball fet després de tancar l'auditoria original** (mentre s'espera l'aprovació de la PR de WI-03/WI-13), abans d'obrir-la. Suite completa (`audit:responsive`, 4 dispositius) re-executada sobre `fix/audit-tap-targets`: 20/20 tests verds, cap regressió d'overflow. Inspeccionant els JSON de mètriques (tap targets/overflow/consola) més enllà de les assercions `soft` es van trobar 3 punts que es van escapar de l'abast ja aprovat de WI-03/WI-13 (mateix root cause, mateixa branca) — **corregits en aquest mateix commit**: (1) `NodesTabComponent` ("Nodes extra") no tenia el guard mòbil que WI-13 sí va aplicar a `PinyesTabComponent`/`TroncsTabComponent` — mateix disseny trencat (`aside w-52` fix + canvas `flex-1`), confirmat amb Playwright: canvas a 800px en viewport de 393px; aplicat el mateix patró `matchMedia`. (2) Login del dashboard (`login.component.html`) no tenia el `h-6` que WI-03 sí va aplicar al login de la PWA (PW-L3/L4) — alçada real 20px; ara 24px, verificat al navegador amb `getBoundingClientRect`. (3) Enllaç "Lloc:" del detall d'Event (`event-detail.component.html:145`) és el mateix bug de `leading-6` documentat a la fila de WI-03 (l'alçada de caixa d'un `inline` no la infla el `line-height`) però es va escapar d'aquell fix — ara `inline-flex items-center min-h-6`. TDD: tests nous a `nodes-tab.component.spec.ts` (guard mòbil), `login.component.spec.ts` (alçada `h-6`) i `event-detail.component.spec.ts` (enllaç d'ubicació), vermell→verd. Verificat: suite dashboard 1324/1326 (2 skips previs), lint 0 errors, confirmació visual/JS al navegador (login 24×24 real; guard "Nodes extra" mostrat a 375px i canvas intacte a desktop). Mateixa branca `fix/audit-tap-targets`. **A més**, es van detectar 3 troballes noves fora de l'abast de qualsevol WI aprovat, registrades com WI-21/22/23 pendents de decisió (no tocades ara). |
...
| RE-AUDIT 2026-07-22 | RE-01 (teclat confirmació ~13px/tecla a mòbil) | WI-21 |
| RE-AUDIT 2026-07-22 | RE-02 (inputs de cerca transversals ~20px) | WI-22 |
| RE-AUDIT 2026-07-22 | RE-03 (400 a `/persons/new`) | WI-23 |
