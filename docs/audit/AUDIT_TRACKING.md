...
- **Aterratge de la infra d'auditoria a `develop`:** ¿PR pròpia de `story/playwrite-test` → `develop` primer (perquè informes + seguiment + specs hi siguin), i després les PR de fix per sobre? O incloure aquest fitxer a la primera PR de fix?
