...
| WI-09 | **Nav tablet**: etiquetes visibles / tooltip real (no només icones) | Shell | 🟠 | S | UX-M4 | ✅ | 🟣 | `fix/audit-nav-tablet-labels` (pendent d'obrir PR) |
...
| 2026-07-22 | WI-09 | ✅ Aprovat → 🟣 En PR | Etiquetes de navegació visibles en tablet (UX-M4): `TabNavComponent` amagava l'etiqueta fins a `lg` (1024px) i només mostrava un `sr-only` (mai visible en tàctil) entre `sm` i `lg`. **Fix:** breakpoint baixat de `lg` a `md` (768px) — `hidden md:inline` per l'etiqueta visible, `md:hidden sr-only` per l'accessible nom entre `sm` i `md` (640-767px, encara icona sola). Sense JS/matchMedia, purament CSS Tailwind, consistent amb el patró existent del component. TDD: nou `tab-nav.component.spec.ts` (no existia cap test previ), vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 700px (icona sola, esperat), 768px (6 etiquetes visibles sense overflow), 1280px (sense regressió). Branca `fix/audit-nav-tablet-labels`. |
