...
| WI-08 | **Detall Event**: assistència com a llista de fitxes en mòbil | Events | 🟠 | M | EV-M2 | ✅ | 🟣 | `fix/audit-event-attendance-list` (pendent d'obrir PR) |
...
| 2026-07-22 | WI-08 | ✅ Aprovat → 🟣 En PR | Llista d'assistència com a targetes en mòbil al detall d'Event (EV-M2): la taula de 557px (5 columnes: Àlies, Nom, Estat, Resposta, Etiquetes) scrollejava horitzontalment a 393px. **Disseny:** taula fêta a mà (no `app-data-table`), fora d'abast de WI-02 — es reutilitza el mateix patró `cardMode` (`matchMedia('(max-width:1023.98px)')`, fallback taula sense `matchMedia`) però amb un bloc de targetes dedicat en lloc de migrar a `app-data-table`, ja que el contingut de cada cel·la (enllaços, múltiples badges, filtre per etiqueta) no encaixa bé en el renderitzador genèric etiqueta→valor. Targeta = àlies+nom (clicables) a l'esquerra, badge d'estat (clicable, obre el mateix modal d'edició) a la dreta, resposta i etiquetes a sota. Reutilitza `getStatusLabel`/`getStatusBadgeClass`/`navigateToPerson`/`filterByPosition`/`openAttendanceEdit` sense canvis. TDD: nou bloc de tests amb TestBed complet (el fitxer només tenia tests purs sense DOM fins ara) — 3 tests nous (taula per defecte, targetes en `cardMode`, badge d'estat clicable), vermell→verd. Verificat: suite dashboard verda, lint 0 errors, build ✓, confirmació visual a 393px (3 targetes, sense overflow) i 1280px (taula intacta, sense regressió). Branca `fix/audit-event-attendance-list`. |
