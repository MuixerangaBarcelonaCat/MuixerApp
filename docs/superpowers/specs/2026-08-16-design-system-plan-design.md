...
**7.2 — Sub-phase B: Dashboard rollout.** Page-by-page, roughly in ascending complexity: `home` → `persons`/`rehearsals`/`performances` lists → detail pages → `config` → `pinyes` module last (most complex). Each page's pass covers both swapping in Tier 1/2 primitives *and* restyling any Tier 3 single-app component that page touches (3.2) — one visit, not two. The `pinyes` step explicitly includes restyling the Konva canvas itself (`libs/pinyes-render`) per 2.3 — not just the surrounding chrome — since it's shared with the PWA's read-only projection view, getting it right here also covers 7.3. Each increment gets a real browser check before merge.
