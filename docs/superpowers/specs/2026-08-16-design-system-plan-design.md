...
|---|---------|-----------|-------|
| 1 | `/home` | 3 buttons, 20 `.card`-classed elements, 1 empty-state | Lightest page — the nav-tile cards are the first real `lib-card sash="title"` usage outside `/design-system` and `/config`. Good warm-up. |
| 2 | `auth` (login, forgot/reset password) | 3 files, 3 buttons, 5 inputs | Small, but the highest-visibility screen in the app — first thing every session sees. Already covered once in 7.1; this is the rest of the feature (forgot/reset password). |
