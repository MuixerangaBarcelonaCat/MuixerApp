...
| 3 | `sync` + per-module `*-sync` components (`persons/person-sync`, `events/event-sync`) | ~4 buttons each, no modals | Small and mechanical, no modal-rewrite risk — a low-stakes place to build page-by-page rhythm before the bigger features. |
| 4 | `communication` (news list + editor) | 8 buttons, 1 modal, 1 empty-state | Smallest feature with a real modal in it — good place to work out the modal-rewrite pattern (see below) before the much bigger modal counts in `persons`/`config`/`events`/`pinyes`. |
