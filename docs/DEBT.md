...
| F13 | La restricció "Passa llista" nomès el dia de l'event està desactivada temporalment (`ENFORCE_ROLL_CALL_DATE_RESTRICTION = false`) per provar en real el refresc periòdic i les estadístiques d'assistència de l'event | `apps/pwa/src/app/features/events/event-detail/event-detail.component.ts` | Tornar a `true` un cop verificat en desplegament |
