...
| F10 | `lib-alert` (`libs/ui`) ja existeix però ~30 usos de `alert alert-*` en cru al dashboard (majoritàriament tires d'error de formulari `text-sm py-2`) i el bàner de dependents pendents de `home` a la PWA encara no s'hi han migrat | dashboard `features/*` (auth, modals, `import-pinya-modal`, `segment-conflict-panel`…), `apps/pwa/.../home` | Migrar de forma oportunista quan es toqui cada pàgina; no és urgent (el `.alert` en cru funciona, només deriva) |
