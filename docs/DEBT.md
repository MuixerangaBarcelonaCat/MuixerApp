...
| F10 | El bàner de dependents pendents de `home` a la PWA encara fa servir `alert alert-info` en cru (no `lib-alert`); dos usos més queden deliberadament en cru per no encaixar amb `lib-alert` (bàner flotant `push-permission-banner` a la PWA, indicador d'estat amb spinner de `event-detail` al dashboard, i el bàner flotant de `nodes-tab`) | `apps/pwa/.../home`, `apps/pwa/.../push-permission-banner`, `apps/dashboard/.../event-detail`, `nodes-tab` | El bàner de `home` es migra al pas 4 del rollout PWA; els altres són arquetipus diferents (flotant / progressiu) |
