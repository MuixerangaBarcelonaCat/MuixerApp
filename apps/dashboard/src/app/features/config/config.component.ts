    ...
    <div class="space-y-4 max-w-3xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <lib-card sash="title" title="Usuaris" [icon]="ICON_USERS" routerLink="users">
          <p class="text-sm text-base-content/60">Gestió d'accés i rols</p>
        </lib-card>

        <lib-card sash="title" title="Etiquetes" [icon]="ICON_TAG" routerLink="tags">
          <p class="text-sm text-base-content/60">Categories i etiquetes</p>
        </lib-card>

        <lib-card sash="title" title="Temporades" [icon]="ICON_SEASONS" routerLink="seasons">
          <p class="text-sm text-base-content/60">Gestió de temporades</p>
        </lib-card>

        @if (auth.isAdmin()) {
          <lib-card sash="title" title="Privacitat i legal" [icon]="ICON_LEGAL" routerLink="legal">
            <p class="text-sm text-base-content/60">Política de privacitat i clàusules</p>
          </lib-card>
        }

        @if (auth.isAdmin()) {
          <lib-card sash="title" title="Design System" [icon]="ICON_DESIGN_SYSTEM" routerLink="/design-system">
            <p class="text-sm text-base-content/60">Tokens i llibreria de components libs/ui</p>
          </lib-card>
        }
