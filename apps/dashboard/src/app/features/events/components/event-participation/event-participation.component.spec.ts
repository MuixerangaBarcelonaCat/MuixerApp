...
describe('EventParticipationComponent', () => {
  ...
  describe('filters', () => {
    ...
    it('filtra les files per qualsevol dels grups seleccionats', async () => {
      const troncTag = { id: 't1', name: 'Segona', slug: 'segona', color: '#111', category: TagCategory.TRONC, positionTypes: [] };
      const pinyaTag = { id: 't2', name: 'Vents', slug: 'vents', color: '#222', category: TagCategory.PINYA, positionTypes: [] };
      const xicallaTag = { id: 't3', name: 'Xicalla', slug: 'xicalla', color: '#333', category: TagCategory.XICALLA, positionTypes: [] };
      const response = buildResponse({
        persons: [
          makePerson('p1', 'PERSIANA', {}, { positions: [troncTag] }),
          makePerson('p2', 'GRILLAT', {}, { positions: [pinyaTag] }),
          makePerson('p3', 'BUFAT', {}, { positions: [xicallaTag] }),
        ],
      });
      const fixture = await setup(response);

      fixture.componentInstance.onCategoriesChange([TagCategory.TRONC, TagCategory.XICALLA]);

      const aliases = fixture.componentInstance.filteredRows().map((r) => r.alias);
      expect(aliases).toEqual(expect.arrayContaining(['PERSIANA', 'BUFAT']));
      expect(aliases).not.toContain('GRILLAT');
    });

    it('does not filter rows when no group is selected', async () => {
      const troncTag = { id: 't1', name: 'Segona', slug: 'segona', color: '#111', category: TagCategory.TRONC, positionTypes: [] };
      const response = buildResponse({
        persons: [
          makePerson('p1', 'PERSIANA', {}, { positions: [troncTag] }),
      ...
      fixture.componentInstance.onCategoriesChange([]);

      expect(fixture.componentInstance.filteredRows().map((r) => r.alias)).toEqual(
        expect.arrayContaining(['PERSIANA', 'GRILLAT']),
      );
    });

    it('lists every selected group in the active-filter chip and clearing it empties the selection', async () => {
      const fixture = await setup();

      fixture.componentInstance.onCategoriesChange([TagCategory.TRONC, TagCategory.XICALLA]);
      const chip = fixture.componentInstance.activeFilters().find((f) => f.key === 'category');
      expect(chip?.label).toContain('Tronc');
      expect(chip?.label).toContain('Xicalla');

      fixture.componentInstance.removeFilter('category');
      expect(fixture.componentInstance.categoryFilters()).toEqual([]);
    });
