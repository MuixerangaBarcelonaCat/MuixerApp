...
describe('FigurePickerModalComponent', () => {
  ...
  it('filters figures by search query', () => {
    component.figures.set([makeFigure({ name: 'pd4' }), makeFigure({ id: 'fig-2', name: 'pilar' })]);
    component.search.set('pil');
    expect(component.filteredFigures()).toHaveLength(1);
    expect(component.filteredFigures()[0].name).toBe('pilar');
  });
