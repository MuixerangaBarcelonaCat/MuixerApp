...
/** Spreads simultaneous refetches so every open projection doesn't hit the API at once. */
...
export class SegmentChangesService {
   ...
   *
   * Resolves the segment id lazily so prev/next navigation reuses the same
   * connection — the stream is keyed by event, not by segment.
   */
  watch(eventId: string, currentSegmentId: () => string): Observable<void> {
    return new Observable<void>((subscriber) => {
      ...
      const source = new EventSource(
        `${environment.apiUrl}/me/events/${eventId}/changes?token=${encodeURIComponent(token)}`,
      );
