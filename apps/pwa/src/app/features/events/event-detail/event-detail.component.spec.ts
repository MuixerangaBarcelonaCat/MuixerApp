...
describe('EventDetailComponent', () => {
  ...
  describe('roll-call link ("Passa llista")', () => {
    ...
    // ponytail: day-of restriction temporarily disabled (docs/DEBT.md F13) — these two document
    // that it's currently shown regardless of date; flip back to "toBeFalsy()" when F13 is resolved.
    it('shows it for TECHNICAL on a future event (restriction disabled, F13)', async () => {
      fixture = await setup(of({ ...MOCK_DETAIL, date: '2099-01-01' }), of([]), UserRole.TECHNICAL);
      expect(fixture.nativeElement.querySelector('[data-testid="roll-call-link"] a')).toBeTruthy();
    });

    it('shows it for TECHNICAL on a past event (restriction disabled, F13)', async () => {
