...
export class MeService {
  ...
  /**
   * Cheap poll target for the projection screen: the latest assignment change timestamp for the
   * segment, so a client can detect "something changed" without refetching the full projection.
   */
  async getSegmentProjectionVersion(segmentId: string): Promise<{ updatedAt: string | null }> {
    const row = await this.nodeAssignmentRepository
      .createQueryBuilder('assignment')
      .select('MAX(assignment."updatedAt")', 'max')
      .where('assignment."segmentId" = :segmentId', { segmentId })
      .getRawOne<{ max: Date | null }>();

    return { updatedAt: row?.max ? new Date(row.max).toISOString() : null };
  }
