...
describe('MeService', () => {
  ...
  describe('getSegmentProjectionVersion', () => {
    it('returns the latest assignment updatedAt for the segment as ISO string', async () => {
      const updatedAt = new Date('2026-07-01T10:00:00.000Z');
      const mockQb = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        getRawOne: jest.fn().mockResolvedValue({ max: updatedAt }),
      };
      nodeAssignmentRepo.createQueryBuilder.mockReturnValue(mockQb as never);

      const result = await service.getSegmentProjectionVersion('seg-1');

      expect(mockQb.where).toHaveBeenCalledWith('assignment."segmentId" = :segmentId', {
        segmentId: 'seg-1',
      });
      expect(result).toEqual({ updatedAt: updatedAt.toISOString() });
    });

    it('returns null when the segment has no assignments yet', async () => {
      const mockQb = {
        select: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
