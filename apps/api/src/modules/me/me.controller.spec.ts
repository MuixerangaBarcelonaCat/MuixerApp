...
describe('MeController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            ...
            getSegmentProjectionVersion: jest.fn(),
  ...
  describe('getSegmentProjectionVersion', () => {
    it('delegates to MeService with the segment id', async () => {
      const expected = { updatedAt: '2026-07-01T10:00:00.000Z' };
      meService.getSegmentProjectionVersion.mockResolvedValue(expected);

      const result = await controller.getSegmentProjectionVersion('seg-1');

      expect(meService.getSegmentProjectionVersion).toHaveBeenCalledWith('seg-1');
      expect(result).toEqual(expected);
    });
  });
