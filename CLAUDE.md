...
| `season` · `event` | seasons, events and attendance. `AttendanceSweepService` (cron `EVERY_DAY_AT_3AM`): for every `ACTUACIO` dated today, flips `ANIRE` → `ASSISTIT` (performances have no arrival tablet); never touches `ASSAIG`/`PENDENT`/`NO_VAIG`/`respondedAt`, idempotent |
