...
| `me` | member-scoped API consumed by the PWA (own events/attendance, published segments + projection). `PersonProfileSummary` includes `seasonAttendance` (assajos/actuacions attended vs. total, current season). `GET events/:eventId/segments/:segmentId/projection/version` is a cheap poll target (latest assignment-change timestamp) so the PWA projection screen avoids refetching the full projection on every poll tick. `GET events/:id/attendance-stats` (TECHNICAL/ADMIN only) returns attendance counts by status, adults vs. xicalla |
